...
template <> struct fmt::formatter<DestructorContext>: formatter<string_view> {
  // parse is inherited from formatter<string_view>.
  template <typename FormatContext>
  auto format(DestructorContext c, FormatContext& ctx) {
    string_view name = "unknown";
    switch (c) {
    case DestructorContext::kEvictedFromRAM:   name = "kEvictedFromRAM"; break;
    case DestructorContext::kEvictedFromNVM:   name = "kEvictedFromNVM"; break;
    case DestructorContext::kRemovedFromRAM:   name = "kRemovedFromRAM"; break;
    case DestructorContext::kRemovedFromNVM:   name = "kRemovedFromNVM"; break;
    }
...
class ItemRecords {
 ...
 public:
  ...
  void addItemRecord(ItemHandle& handle) {
    ...
    records.resize(std::max<size_t>(idx / itemRecords_.size() + 1, records.size()));
